...
@section('content')
    ...
    <div class="card content-card"><div class="card-header"><h5>Dados da justificativa</h5></div><div class="card-body">
        @php
            $campos = [['Descrição', $justificativa->descricao, 'col-12'], ['ID', $justificativa->id], ['Colaborador', $justificativa->colaborador?->nome], ['Categoria', $justificativa->categoria?->nome], ['Situação', $justificativa->status], ['Data de cadastro', $justificativa->created_at?->format('d/m/Y H:i')], ['Última alteração', $justificativa->updated_at?->format('d/m/Y H:i')]];
