    ...
    <form method="POST" action="{{ route('colaboradores.update', $colaborador) }}">
        ...
        <div class="card content-card">
            ...
            <div class="card-body">
                ...
                <div class="row g-3">
                    <div class="col-md-3"><label class="form-label" for="id">ID do usuário no GI</label><input id="id" class="form-control" value="{{ $colaborador->id }}" disabled></div>
                    <div class="col-md-3"><label class="form-label" for="perfil_id">ID do perfil</label><input id="perfil_id" class="form-control" value="{{ $colaborador->perfil_id }}" disabled></div>
                    <div class="col-md-3"><label class="form-label" for="perfil">Perfil</label><input id="perfil" class="form-control" value="{{ $colaborador->perfil }}" disabled></div>
                    <div class="col-md-3"><label class="form-label" for="ativo">Status</label><select id="ativo" name="ativo" class="form-select" required><option value="1" @selected((string) old('ativo', (int) $colaborador->ativo) === '1')>Ativo</option><option value="0" @selected((string) old('ativo', (int) $colaborador->ativo) === '0')>Inativo</option></select></div>
